...
REAL_EXECUTION_ENABLED = False  # conexión REAL sí; envío automático se habilitará solo tras completar protección broker-side
...
def real_validate_candidate(signal):
    ...
    if strategy not in REAL_ALLOWED_STRATEGIES:
        reasons.append("REAL solo permite PCS o CCS")
    try:
    ...
    for signal in signals:
        ...
        try:
            ...
            if record:
                ...
                exit_status = str(record.get("exit_status") or "").lower()
                if (
                    str(signal.get("status")) == "CLOSED"
                    and entry_status == "filled"
                    and not record.get("exit_order_id")
                    and exit_status not in {"filled", "pending", "open", "submitted"}
                ):
            ...
            if signal_time < cutoff or not entry_window_open or daily_loss_locked:
                continue
            ...
            successful_entries = [
                ...
                if str(r.get("entry_status") or "").lower() in {"filled", "open", "pending", "submitted"}
            ]
            ...
            # La operación anterior debe estar FILLED antes de considerar otra.
            ...
            # Errores de red/API obligan a reconciliar antes de la próxima entrada.
            message = str(exc).lower()
            if any(term in message for term in ("conectar", "timeout", "timed out", "network", "url", "http 401", "http 403", "http 429", "tradier paper http")):
...
if page == "📈 Mercado":
    ...
    st.caption("Cuenta de dinero real · reglas de riesgo ultra-conservadoras para capital inicial de $2,000")
    ...
    st.markdown(
        """
        - **Solo PCS y CCS.** Iron Condor, Iron Butterfly, IC Box y cualquier otra estructura quedan bloqueadas en REAL.
        ...
        - **Máximo una posición REAL abierta a la vez.** No se permite una entrada nueva hasta que la anterior esté FILLED, VERIFIED y PROTECTED o completamente cerrada.
        - Al alcanzar **+$50 de P/L realizado en el día**, se bloquean nuevas entradas REAL hasta la siguiente sesión.
    ...
    s3.metric("Envío automático", "BLOQUEADO" if not REAL_EXECUTION_ENABLED else "HABILITADO")

    if connected:
        st.info(
            "La cuenta REAL ya puede quedar conectada y validada aquí. Por seguridad, esta versión todavía NO envía órdenes reales automáticamente: "
            "primero terminaremos y probaremos la protección broker-side TP/SL para spreads, la reconciliación y el armado/desarmado seguro. "
            "Después habilitaremos el interruptor REAL con estas reglas duras."
        )
    else:
...
if page not in {"🤖 Auto Trading SIM", "💵 Auto Trading REAL"}:
    st.caption("SPX Defender analiza y registra operaciones. Auto Trading SIM usa Sandbox; Auto Trading REAL permanecerá bloqueado hasta completar las validaciones de seguridad.")
