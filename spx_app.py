...
PAPER_ALLOWED_STRATEGIES = {"PCS", "CCS", "Iron Condor"}
...
IDEAS_DAILY_LOSS_LIMIT = 500.0
AUTO_IDEA_BLOCKED_STRATEGIES = {"Iron Butterfly"}
...
def automatic_ideas_daily_risk_status(trading_day, symbol):
    """Ideas: stop diario -$500 y trailing lock $200 después de alcanzar +$500."""
    ...
    with AutomaticIdeasDatabase() as database:
        ...
        if total_pnl <= -IDEAS_DAILY_LOSS_LIMIT:
            locked = True
            reason = f"PÉRDIDA MÁXIMA DIARIA -${IDEAS_DAILY_LOSS_LIMIT:,.0f}"
        elif active and total_pnl <= peak - DAILY_PROFIT_LOCK_GIVEBACK:
            locked = True
...
def process_paper_autotrading(symbol):
    ...
    daily_loss_locked = daily_pnl <= -PAPER_DAILY_LOSS_LIMIT
    ...
    if daily_loss_locked:
        state = "GESTIÓN ACTIVA · ENTRADAS BLOQUEADAS -$1,000"
...
def render_paper_autotrading_page(symbol):
    ...
    st.info(
        ...
        "PCS/CCS = 2 patas juntas · Iron Condor = 4 patas juntas. IB e IC Box permanecen solo en Ideas, no en Auto SIM. "
    ...
    with st.expander("🛡️ Qué pasa si se cae Internet", expanded=False):
        ...
        st.caption(
            "Las reglas de horario, noticias y pérdida diaria bloquean entradas nuevas, pero no bloquean la revisión y gestión de operaciones ya abiertas."
        )
...
def render_futures_mes_page():
    ...
    st.warning("PRUEBA INICIAL: usa únicamente Sim101 hasta verificar entrada, TP, SL, OCO y cierre.")
    ...
    mes_tf = chart_controls[0].selectbox(
        ...
        key="mes_chart_minutes",
    )
    mes_bars_back = chart_controls[1].selectbox(
        "Barras",
        [60, 120, 240, 480],
        index=1,
        key="mes_chart_bars_back",
    )
    ...
    try:
        ...
        if chart_refresh or st.session_state.get("mes_bridge_ok"):
            mes_chart_payload = _mes_bridge_request(
                "GET",
                st.session_state.get("mes_bridge_url", "http://127.0.0.1:8765"),
                st.session_state.get("mes_bridge_key", "SPXDEFENDER-MES-2026"),
                f"/bars/{urlparse.quote(st.session_state.get('mes_contract', 'MES 09-26'), safe='')}/{int(mes_tf)}/{int(mes_bars_back)}",
                timeout=8.0,
            )
    ...
    if bars_rows:
        ...
        if not df_mes.empty:
            ...
            st.caption(
                f"NinjaTrader · {mes_chart_payload.get('instrument', st.session_state.get('mes_contract', 'MES'))} · "
    ...
    with left:
        ...
        firm = st.text_input("Compañía / modo", value=st.session_state.get("mes_firm", "SIM TEST"), key="mes_firm")
        account = st.text_input("Cuenta NinjaTrader", value=st.session_state.get("mes_account", "Sim101"), key="mes_account")
        mes_contract = st.text_input("Contrato MES NinjaTrader", value=st.session_state.get("mes_contract", "MES 09-26"), key="mes_contract", help="Nombre completo del contrato en NinjaTrader. Para septiembre de 2026: MES 09-26")
        bridge_url = st.text_input("Bridge URL", value=st.session_state.get("mes_bridge_url", "http://127.0.0.1:8765"), key="mes_bridge_url")
        bridge_key = st.text_input("Bridge Key", value=st.session_state.get("mes_bridge_key", "SPXDEFENDER-MES-2026"), type="password", key="mes_bridge_key")
    ...
    with right:
        ...
        c1, c2 = st.columns(2)
        qty = c1.number_input("Contratos MES", min_value=1, max_value=10, value=2, step=1, key="mes_qty")
        min_score = c2.number_input("Score mínimo", min_value=80.0, max_value=100.0, value=90.0, step=0.5, key="mes_min_score")
        tp = c1.number_input("TP por trade ($)", min_value=5.0, max_value=1000.0, value=100.0, step=5.0, key="mes_tp")
        sl = c2.number_input("SL por trade ($)", min_value=5.0, max_value=2000.0, value=100.0, step=5.0, key="mes_sl")
        ...
        auto_on = st.toggle("AUTO MES", value=bool(st.session_state.get("mes_auto_on", False)), key="mes_auto_on")
        if auto_on:
            st.warning("AUTO MES está activado. Mantén Sim101 durante las pruebas.")
        else:
            st.info("AUTO MES está OFF. Puedes probar la conexión sin enviar órdenes.")

    # Motor automático MES. Se ejecuta periódicamente mientras esta página esté abierta.
    def mes_auto_cycle():
        now = market_time()
        start_time = now.replace(hour=9, minute=30, second=0, microsecond=0)
        st.caption(f"Auto MES · {now.strftime('%I:%M:%S %p')} ET · inicio permitido 9:30 AM")

        if not bool(st.session_state.get("mes_auto_on", False)):
            st.info("Auto MES en espera: OFF.")
            return
        if str(account or "").strip().lower() != "sim101":
            st.error("Durante las pruebas, Auto MES solo puede operar Sim101.")
            return
        if now.weekday() >= 5 or now < start_time:
            st.info("Auto MES esperando las 9:30 AM ET.")
            return
        if not market_is_open(now):
            st.info("Auto MES: mercado regular cerrado.")
            return

        blackout, event = high_impact_news_blackout(now)
        if blackout:
            label = str((event or {}).get("name") or (event or {}).get("event") or "noticia de alto impacto")
            st.warning(f"Auto MES bloqueado por noticias: {label}.")
            return
        ...
        try:
            ...
            st.error(f"Auto MES sin Bridge: {exc}")
            return

        if not _mes_position_is_flat(live_pos):
            current = (live_pos.get("position") or {}).get("status", live_pos.get("status", "ACTIVA"))
            st.info(f"Auto MES esperando cierre de la posición actual: {current}.")
            return

        signal = _best_mes_futures_signal(min_score)
        if not signal:
            st.info(f"Auto MES buscando PCS/CCS con score ≥ {float(min_score):.1f}.")
            return

        signal_id = str(signal.get("id") or "")
        if not signal_id:
            st.warning("La mejor señal no tiene ID; no se enviará para evitar duplicados.")
            return

        used = set(st.session_state.get("mes_used_signal_ids", []))
        if signal_id in used:
            st.info("La mejor señal actual ya fue utilizada; esperando una señal nueva.")
            return
        ...
        st.success(f"Señal lista: {strategy} {score:.1f} → {direction} MES")
        ...
        try:
            ...
            st.success(f"AUTO MES enviado: {strategy} {score:.1f} → {direction} {int(qty)} MES")
        except Exception as exc:
            st.error(f"Auto MES no pudo enviar la orden: {exc}")

    if auto_on:
        use_fragment(mes_auto_cycle, 5)
    else:
        mes_auto_cycle()
