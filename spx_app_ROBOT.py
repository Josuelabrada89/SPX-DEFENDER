...
PAPER_ALLOWED_STRATEGIES = {"PCS", "CCS", "Iron Condor", "Iron Butterfly"}
...
def paper_autotrading_credentials():
    token = str(
        st.session_state.get("tradier_paper_token")
        or configured_application_value("TRADIER_PAPER_TOKEN", "")
        or ""
    ).strip()
    account_id = str(
        st.session_state.get("tradier_paper_account_id")
        or configured_application_value("TRADIER_PAPER_ACCOUNT_ID", "")
        or ""
    ).strip()
...
def process_paper_autotrading(symbol):
    if not bool(st.session_state.get("paper_autotrading_enabled", False)):
...
def render_paper_autotrading_page(symbol):
    ...
    if "tradier_paper_token" not in st.session_state:
        st.session_state["tradier_paper_token"] = str(
            configured_application_value("TRADIER_PAPER_TOKEN", "") or ""
        ).strip()
    if "tradier_paper_account_id" not in st.session_state:
        st.session_state["tradier_paper_account_id"] = str(
            configured_application_value("TRADIER_PAPER_ACCOUNT_ID", "") or ""
        ).strip()
    ...
    st.session_state.setdefault("paper_autotrading_enabled", False)
    ...
    st.info(
        ...
        "PCS/CCS = 2 patas juntas · Iron Condor/Iron Butterfly = 4 patas juntas. "
        "Antes del preview, el bot bloquea cualquier señal que reutilice una pata ya abierta o trabajando en Paper."
    )

    token_default = str(configured_application_value("TRADIER_PAPER_TOKEN", "") or "")
    account_default = str(configured_application_value("TRADIER_PAPER_ACCOUNT_ID", "") or "")
